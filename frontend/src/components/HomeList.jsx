...
function HouseList() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      item && 
      ((item.borough && item.borough.toLowerCase().includes(searchTerm.toLowerCase())) || 
      (item.price_per_year && item.price_per_year.toLowerCase().includes(searchTerm.toLowerCase())))
      );
